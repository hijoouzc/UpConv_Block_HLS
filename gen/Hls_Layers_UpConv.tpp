...
// ============================================================
// UPCONV FUSED ROW — Templatized with New Architecture (DATAFLOW + URAM + Fabric DSP)
// ============================================================

template<int PEs, int C_OUT, int CI_WORDS>
static void uc_load_tile(const data_256_t* W_ptr, int tile, data_256_t wbuf[PEs][9 * CI_WORDS]) {
...
#pragma HLS PIPELINE II=1
                wbuf[tc][k_cnt * CI_WORDS + ci_cnt] = W_ptr[co * 9 * CI_WORDS + kci];
                ci_cnt++;
...
static void uc_compute_tile(data_256_t wbuf[PEs][9 * CI_WORDS], data_256_t* x_buf,
                            data_t row_acc[W_OUT][C_OUT_PAD], int tile, int ho) {
#pragma HLS INLINE off
    const int S = 2, PAD = 1;
    int co_base = tile * PEs;
...
#pragma HLS UNROLL
    ...
    // [DBUF-DATAFLOW] PIPO double-buffer
    TILE_LOOP: for (int tile = 0; tile < N_TILES; tile++) {
#pragma HLS DATAFLOW
        data_256_t wbuf[PEs][9 * CI_WORDS];
#pragma HLS BIND_STORAGE variable=wbuf type=ram_t2p impl=uram
#pragma HLS ARRAY_PARTITION variable=wbuf complete dim=1
        uc_load_tile<PEs, C_OUT, CI_WORDS>(W_ptr, tile, wbuf);
        uc_compute_tile<PEs, H_IN, W_IN, CI_WORDS, W_OUT, C_OUT, C_OUT_PAD>(wbuf, x_buf, row_acc, tile, ho);
    }

    data_256_t b_buf[C_WORDS_OUT], g_buf[C_WORDS_OUT], be_buf[C_WORDS_OUT];
